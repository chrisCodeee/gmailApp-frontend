		...
		<>
			<InboxMessageBodyWrapper className="ms-xl-5 d-xl-flex">
				...
				<div className="d-block d-xl-none">
					{items.subject ? <div style={{ textAlign: "left" }}>{items.subject.length >= 75 ? items.subject.slice(0, 35) : items.subject}</div> : <div>(no subject)</div>}

					<div className="d-flex justify-content-between align-items-center">
						<div style={{ textAlign: "left" }}>{items.subject.length < 15 ? `${items.body.slice(0, 110)}` : items.subject.length < 75 ? `${items.body.slice(0, 40)}${items.body && "..."}` : items.subject.length >= 75 ? `${items.body.slice(0, 35)}${items.body && "..."}` : items.body}</div>
